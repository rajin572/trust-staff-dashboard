...
const OTPVerify = () => {
  ...
  const forgottenEmail = JSON.parse(
    Cookies.get("trustStaffDashboard_forgetEmail") || "null"
  );
  ...
  const handleOTPSubmit = async () => {
    if (otp.length === 4) {
      ...
      if (res?.statusCode === 200) {
        Cookies.remove("trustStaffDashboard_forgetToken");
        Cookies.remove("trustStaffDashboard_forgetEmail");
        Cookies.set(
          "trustStaffDashboard_forgetOtpMatchToken",
